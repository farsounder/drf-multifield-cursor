...
from rest_framework.pagination import Cursor, CursorPagination, BasePagination
from rest_framework.permissions import AllowAny
from rest_framework.serializers import ModelSerializer
from rest_framework.test import APIRequestFactory

from tests.models import ExamplePaginationModel
from drf_multifield_cursor.pagination import MultiFieldCursorPagination
        ...
        if link:
            ...
            cursor_dict = dict(
                parse.parse_qsl(
                    base64.decodebytes(
                        (
                            parse.parse_qs(parse.urlparse(link).query)["cursor"][0]
                        ).encode()
                    )
                )
            )
...
def _test_impl(
    page_size, offset, cls: BasePagination, use_tuple_comparison: bool = False
):
    PaginationCls = type(
        "PaginationCls",
        (cls,),
        dict(
            ...
            max_page_size=20,
            use_tuple_comparison=use_tuple_comparison,
        ),
    )
    ...
    while next_cursor:
        assert (
            expected_result[position : position + len(response.data["results"])]
    ...
    while prev_cursor:
        assert (
            expected_result[position - len(response.data["results"]) : position]
            == response.data["results"]
        )
        position -= len(response.data["results"])
...
@pytest.mark.xfail(
    reason="If these pass you don't need to use MultiFieldCursorPagination"
)
...
@pytest.mark.parametrize(
    ...
    [(6, 2), (2, 6), (5, 3), (3, 5), (5, 5)],
    ids=[
        "page_size_divisor_of_offset_default",
        "page_size_multiple_of_offset_default",
        "page_size_uneven_divisor_of_offset_default",
        "page_size_uneven_multiple_of_offset_default",
        "page_size_same_as_offset_default",
    ],
)
def test_filtered_items_are_paginated_default_pagination(page_size, offset):
    _test_impl(page_size, offset, CursorPagination)


@pytest.mark.django_db
@pytest.mark.parametrize(
    "page_size,offset,use_tuple_comparison",
    [
        (6, 2, False),
        (2, 6, False),
        (5, 3, False),
    ...
    ids=[
        ...
        "page_size_divisor_of_offset_tuple",
        "page_size_multiple_of_offset_tuple",
        "page_size_uneven_divisor_of_offset_tuple",
        "page_size_uneven_multiple_of_offset_tuple",
        "page_size_same_as_offset_tuple",
    ],
)
def test_filtered_items_are_paginated_multifield_pagination(
    page_size, offset, use_tuple_comparison
):
    _test_impl(page_size, offset, MultiFieldCursorPagination, use_tuple_comparison)
